...
- Inter-line join uses `\n` (not space) to preserve original PDF line breaks; `\n` has zero width so no pdfRunWidth adjustment is needed. **Important**: `\n` inside a run is the PDF-internal line wrap marker — NOT a semantic hard break. Downstream `GreedyLineBreaker` treats it as a soft break opportunity; `ParagraphLayout.flattenRuns()` converts it to a real space so measurement / rendering / export all see a word separator with natural space width. (An earlier "force hard break at `\n`" behavior orphaned trailing words when an edit slightly widened an earlier line — e.g. `ipsum → Apsum` orphaning `nec`.) True paragraph breaks live BETWEEN paragraphs in the DocumentModel, not inside a run
