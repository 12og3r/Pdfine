...
- Inter-line join uses `\n` (not space) to preserve original PDF line breaks; `\n` has zero width so no pdfRunWidth adjustment is needed. `\n` inside a run is a **hard line-boundary marker** (force-break in `GreedyLineBreaker`): letting it be a soft break let the greedy packer fuse short PDF rows onto the preceding row on edit-mode entry when `bounds.width` was wider than the individual row (e.g. an "Employee Name: …" cell absorbing "NRIC/FIN …"). True paragraph breaks live BETWEEN paragraphs in the DocumentModel, not inside a run; user-typed newlines also create a new Paragraph via `EditCommands`, so any `\n` inside a run originated here and should be preserved as a row boundary
...
- Records `block.firstBaselineY = group.lines[0].y` (first-line baseline in layout coords). `EditorCore.adjustBoundsToFontAscent` uses it to compute `bounds.y = firstBaselineY - ascent` so Canvas `fillText` lands exactly on the pdfjs raster's baseline on edit-mode entry. Without this, the correction defaults to `fontSize - ascent`, which is only exact when `item.height == fontSize`; pdfjs reports `textItem.height` as the glyph bbox height, and for CJK-capable / condensed / display faces that differs from fontSize by a pixel or two — enough to make the block visibly jump on double-click and leave a raster ghost leaking around the white overlay (reads as a font-weight change)
