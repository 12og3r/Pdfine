...
- `flattenRuns(paragraph, fontManager)` → CharInfo[] (flat character array with styles and optional `pdfWidth`); computes proportional per-char widths from `pdfRunWidth`/`pdfLineWidths` using `computeProportionalWidths()`, then **stores them back** on `run.pdfCharWidths` (and caches the PDF→canvas ratio on `run.pdfWidthScale`) and clears `run.pdfRunWidth`/`pdfLineWidths` so subsequent layouts (after text insertion) preserve original character widths
- **On subsequent layouts**: chars without a valid `pdfCharWidths[i]` (NaN placeholders from `EditCommands.insertText`, or brand-new runs) fall back to `fontManager.measureChar(...) × run.pdfWidthScale`. Reusing the same scale factor keeps existing and inserted chars in one metric space — otherwise a single-char swap on a line sized exactly to its block width (e.g. "PDFObject" → "PDFAbject" in the example_en.pdf title) can tip the line past the 0.5 px soft-wrap tolerance, force an unwanted wrap, and grow the block vertically onto the paragraph below.
- **`\n` is kept verbatim** inside `flattenRuns` (via `charInfoFor`). The parser writes `\n` between PDF lines to preserve the source row structure and `GreedyLineBreaker` force-breaks at it, so `\n` always lands at the end of a laid-out line and gets stripped by the trailing-whitespace trim before `positionGlyphs` ever sees it. Previously this helper converted `\n` → `' '` to avoid visual gluing when content flowed across the marker, but that is impossible now that the breaker never flows across `\n` — and the conversion caused short PDF rows in multi-row blocks (e.g. "Employee Name: …") to absorb the next row's leading words on edit-mode entry, because the merged-into-space marker let the greedy packer keep going past the row boundary
...
- **`\n` is a HARD break.** `\n` inside a run is written exclusively by `TextBlockBuilder` to mark a PDF line boundary (user-typed newlines create a new `Paragraph` in `EditCommands`, not a `\n` inside a run). Treating it as a soft break let the greedy packer pull the next PDF row's leading words onto the preceding row on edit-mode entry whenever `bounds.width` (= width of the widest row) exceeded an individual row's width — visible as "Employee Name: Guan Zhenzhi NRIC/FIN…" in the employment-certificate repro. The forced break is placed AFTER the `\n` so the zero-width marker stays with the line that just ended and `ParagraphLayout`'s trailing-whitespace trim discards it before `positionGlyphs`. An edit that widens a row past its original PDF width causes a natural break within the row; the following `\n` still forces the next row to start on its own line, so the widened row may add one line but never swallows the next row's content
